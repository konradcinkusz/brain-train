...
def render_postep(order) -> str:
    """A grid per benchmark, for plotting the five measurements.

    Five numbers in a column is a table; five points on a grid is a trend, and
    the difference is the whole reason the benchmark sets exist. The y axis
    carries no numbers because the book cannot know them -- one reader's first
    measurement is four minutes and another's is eleven, and a printed scale
    would be wrong for both. The reader writes their own, which also means the
    axis is scaled to THEIR range rather than to a range chosen to make any
    improvement look small.
    """
    out = ["% GENERATED by tools/gen_plan.py -- do not edit.\n"]
    for i, m in enumerate(i for i in order if i.block == 9):
        if i and i % 2 == 0:
            out.append("\n\\par\\vspace{14pt}\n")
        out.append(f"\\btsiatka{{{m.num}}}{{{esc(m.title)}}}\n")
        if i % 2 == 0:
            out.append("\\hfill\n")
    return "".join(out)
...
def render(days, repeats, spare) -> str:
    ...
    out.append("Plan nie zużywa całej książki. Te zestawy zostają na dodatkową "
               "powtórkę w dniu, w którym zostało Ci jeszcze pięć minut --- "
               "albo na piąty miesiąc, jeśli dojdziesz do końca i nie "
               "przestaniesz:\n\n")
...
def main() -> int:
    ...
    # all five wrong at once with nothing able to see it. This is the
    # repository's own rule about tallies, applied to the one place it had not
